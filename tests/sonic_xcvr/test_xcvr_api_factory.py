...
from sonic_platform_base.sonic_xcvr.api.amphenol.aph_bc import CmisAphBcApi
...
class TestXcvrApiFactory(object):
    ...
    @patch('sonic_platform_base.sonic_xcvr.xcvr_api_factory.XcvrApiFactory._get_vendor_name', MagicMock(return_value='AMPHENOL'))
    @patch('sonic_platform_base.sonic_xcvr.xcvr_api_factory.XcvrApiFactory._get_vendor_part_num', MagicMock(return_value='HS33006'))
    def test_create_xcvr_api(self):
        self.api.reader = self.mock_reader
        CmisCodes = MagicMock()
        CmisMemMap = MagicMock()
        XcvrEeprom = MagicMock()
        CmisAphBcApi = MagicMock()
        self.api.create_xcvr_api()
